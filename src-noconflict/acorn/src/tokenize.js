...
import {
    lineBreak, lineBreakG, isNewLine, nonASCIIwhitespace,
} from './whitespace';
...
pp.skipBlockComment = function () {
    const startLoc = this.options.onComment && this.options.locations && this.curPosition();
...
let regexpUnicodeSupport = false;
try { new RegExp('\uffff', 'u'); regexpUnicodeSupport = true; } catch (e) {}

// Parse a regular expression. Some context-awareness is necessary,
// since a '/' inside a '[]' set does not end the expression.

pp.readRegexp = function () {
    let escaped; let inClass; const
        start = this.pos;
    for (;;) {
...
pp.readInt = function (radix, len) {
    const start = this.pos; let
        total = 0;
    for (let i = 0, e = len == null ? Infinity : len; i < e; ++i) {
        const code = this.input.charCodeAt(this.pos); let
            val;
...
pp.readNumber = function (startsWithDot) {
    const start = this.pos; let isFloat = false; const
    ...
    const str = this.input.slice(start, this.pos); let
        val;
...
pp.readCodePoint = function () {
    const ch = this.input.charCodeAt(this.pos); let
        code;
