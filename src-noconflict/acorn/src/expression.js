...
pp.parseExpression = function (noIn, refShorthandDefaultPos) {
    const start = this.markPosition();
    const expr = this.parseMaybeAssign(noIn, refShorthandDefaultPos);
    if (this.type === tt.comma) {
        const node = this.startNodeAt(start);
...
pp.parseMaybeAssign = function (noIn, refShorthandDefaultPos) {
    ...
    const start = this.markPosition();
    const left = this.parseMaybeConditional(noIn, refShorthandDefaultPos);
    ...
    } if (failOnShorthandAssign && refShorthandDefaultPos.start) {
...
pp.parseExprOp = function (left, leftStart, minPrec, noIn) {
    const prec = this.type.binop;
    if (prec != null && (!noIn || this.type !== tt._in)) {
        if (prec > minPrec) {
            const node = this.startNodeAt(leftStart);
            node.left = left;
            node.operator = this.value;
            const op = this.type;
            this.next();
...
pp.parseMaybeUnary = function (refShorthandDefaultPos) {
    if (this.type.prefix) {
        const node = this.startNode(); const
        ...
        if (update) this.checkLVal(node.argument);
             ...
             && node.argument.type === 'Identifier') this.raise(node.start, 'Deleting local variable in strict mode');
...
pp.parseExprAtom = function (refShorthandDefaultPos) {
    ...
    case tt.name:
        const start = this.markPosition();
        const id = this.parseIdent(this.type !== tt.name);
...
pp.parseParenAndDistinguishExpression = function () {
    const start = this.markPosition(); let
        val;
    if (this.options.ecmaVersion >= 6) {
        ...
        const innerStart = this.markPosition(); const exprList = []; let
            first = true;
        const refShorthandDefaultPos = { start: 0 }; let spreadStart; let
            innerParenStart;
        ...
        const innerEnd = this.markPosition();
        this.expect(tt.parenR);
    ...
    if (this.options.preserveParens) {
        const par = this.startNodeAt(start);
        par.expression = val;
...
pp.parseObj = function (isPattern, refShorthandDefaultPos) {
    const node = this.startNode(); let first = true; const
        propHash = {};
    node.properties = [];
    this.next();
    while (!this.eat(tt.braceR)) {
        ...
        const prop = this.startNode(); let isGenerator; let
            start;
        if (this.options.ecmaVersion >= 6) {
            prop.method = false;
        ...
        if (this.eat(tt.colon)) {
            ...
            || (!this.options.allowReserved && this.isReservedWord(prop.key.name))) this.raise(prop.key.start, `Binding ${prop.key.name}`);
...
pp.parseFunctionBody = function (node, allowExpression) {
    const isExpression = allowExpression && this.type !== tt.braceL;
    ...
    // flag (restore them to their old value afterwards).
        const oldInFunc = this.inFunction; const oldInGen = this.inGenerator; const
...
pp.parseExprList = function (close, allowTrailingComma, allowEmpty, refShorthandDefaultPos) {
    ...
    while (!this.eat(close)) {
        ...
        if (allowEmpty && this.type === tt.comma) {
            elts.push(null);
        } else if (this.type === tt.ellipsis) elts.push(this.parseSpread(refShorthandDefaultPos));
    ...
    if (this.type === tt.name) {
        ...
        && ((!this.options.allowReserved && this.isReservedWord(this.value))
         ...
         && (this.options.ecmaVersion >= 6
          || this.input.slice(this.start, this.end).indexOf('\\') == -1))) this.raise(this.start, `The keyword '${this.value}' is reserved`);
